  ...
  app.innerHTML = `
    <main class="public-home">
      ...
      <section class="public-hero-card">
        ...
        <footer>
          © 2026 ${APP_CONFIG.schoolName} · Professional Edition v2.2
        </footer>
      </section>
...
function openStudentLoginModal() {
    ...
    <div class="modal-backdrop" id="studentLoginBackdrop">
      <section class="student-modal student-login-modal">
        <div class="modal-head">
          <div>
            <h3>Student Login</h3>
            <p class="student-login-subtitle">
              Enter your matric number and password to open your result.
            </p>
          </div>
  ...
  document.getElementById("closeStudentLogin").addEventListener("click", close);
  document.getElementById("studentLoginBackdrop").addEventListener("click", event => {
    if (event.target.id === "studentLoginBackdrop") close();
  });
  ...
  document.getElementById("studentLoginForm").addEventListener("submit", async event => {
    ...
    const button = document.getElementById("studentLoginSubmit");
    const message = document.getElementById("studentLoginMessage");

    if (password !== "sch12345") {
      message.innerHTML = `<div class="message error">Incorrect password.</div>`;
      return;
    }
    ...
    try {
      ...
      const exactResult = await getDocs(
        query(
      ...
      if (!exactResult.empty) {
        studentDocument = exactResult.docs[0];
      }
...
function renderPublicYearResult(student, results, year) {
  ...
  document.getElementById("publicPrintYear")?.addEventListener("click", () => printResultDocument("session"));
}
...
function renderPublicTranscript(student, results) {
  ...
  document.getElementById("publicPrintTranscript").addEventListener("click", () => printResultDocument("transcript"));
}
...
async function saveImportToFirestore() {
  ...
  try {
    ...
    const affectedStudentIds = [...new Set(
      pendingImport.students.map(student => safeDocumentId(student.matricNumber))
    )];

    button.textContent = "Calculating CGPA...";

    for (let index = 0; index < affectedStudentIds.length; index += 1) {
      await recalculateSingleStudent(affectedStudentIds[index]);
      progressBar.style.width = `${Math.round(((index + 1) / affectedStudentIds.length) * 100)}%`;
    }

    message.innerHTML = `
      <div class="message success">
        Import completed successfully: ${pendingImport.students.length} students and
        ${pendingImport.students.length} semester result records. GPA, cumulative CGPA,
        carry-overs and academic remarks were calculated automatically.
      </div>
    `;
...
async function viewResultRecord(resultId) {
  try {
    ...
    document.getElementById("printResultBtn").addEventListener("click", () => printResultDocument("semester"));
  } catch (error) {
    alert(error.message);
  }
}
...
function renderYearResult(student, results, year) {
  ...
  document.getElementById("printYear")?.addEventListener("click", () => printResultDocument("session"));
}
...
function renderCompleteTranscript(student, results) {
  ...
  `;
  document.getElementById("printTranscript").addEventListener("click", () => printResultDocument("transcript"));
}
...
async function isAdministrator(user) {
  if (!user || user.isAnonymous) {
    return { allowed: false, reason: "Anonymous accounts cannot open the administrator dashboard." };
  }
  ...
  try {
    ...
    if (!snapshot.exists()) {
      return {
        allowed: false,
        reason: `No administrator document was found at users/${user.uid}.`,
        uid: user.uid,
        email: user.email || ""
      };
    }
    ...
    if (role !== "admin") {
      return {
        allowed: false,
        reason: `The Firestore role is "${snapshot.data().role || ""}", not "admin".`,
        uid: user.uid,
        email: user.email || ""
      };
    }
    ...
    return {
      allowed: false,
      reason: error.message || "The administrator role could not be checked.",
      code: error.code || "",
      uid: user.uid,
      email: user.email || ""
    };
...
// Display the homepage immediately. Firebase may update the page after auth state resolves.
try {
  showPublicHome();
} catch (error) {
  ...
  app.innerHTML = `
    <main class="login-screen">
      <section class="login-card">
        ...
        <h1>SAHLAN Result Bank</h1>
        <div class="message error">
          The application could not start: ${escapeHtml(error.message || error)}
        </div>
      </section>
    </main>
  `;
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!adminCheck.allowed) {
      ...
      <main class="login-screen">
        <section class="login-card">
          ...
          <div class="message error" style="margin-top:18px">
            ${escapeHtml(adminCheck.reason)}
          </div>

          <div class="card" style="margin-top:16px;text-align:left">
            <p><strong>Email:</strong><br>${escapeHtml(adminCheck.email || user.email || "")}</p>
            <p style="margin-top:10px"><strong>Authentication UID:</strong><br>
              <code style="word-break:break-all">${escapeHtml(adminCheck.uid || user.uid)}</code>
            </p>
          ...
          <p style="margin-top:16px;color:var(--muted)">
            In Firestore, create a document in the <strong>users</strong> collection using the exact UID above
            as the Document ID. Add a string field named <strong>role</strong> with the value <strong>admin</strong>.
          </p>

          <button id="adminCheckBackBtn" class="primary-btn" style="width:100%;margin-top:18px">
            Return to Home
          </button>
        </section>
      </main>
    ...
    document.getElementById("adminCheckBackBtn").addEventListener("click", () => signOut(auth));
    return;
  }
